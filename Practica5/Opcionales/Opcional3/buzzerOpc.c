...
const int buttons_gpio[NR_GPIO_BUTTONS] = {22, 4, 27};

/* Array to hold gpio descriptors */
struct gpio_desc* gpio_descriptors[NR_GPIO_BUTTONS];

static int gpio_button_irqn[NR_GPIO_BUTTONS] = {-1, -1, -1};
...
int init_module(void)
{
	...
	struct device *device;
	  ...
	  for (i = 0; i < NR_GPIO_BUTTONS; i++) {
	    ...
	    //Requesting the GPIO
	    if ((err = gpio_request(buttons_gpio[i], gpio_str))) {
	      pr_err("Failed GPIO[%d] %d request\n", i, buttons_gpio[i]);
	      goto err_handle;
	    }

	    /* Transforming into descriptor **/
	    if (!(gpio_descriptors[i] = gpio_to_desc(buttons_gpio[i]))) {
	      pr_err("GPIO[%d] %d is not valid\n", i, buttons_gpio[i]);
	      err = -EINVAL;
	      goto err_handle;
	    }

	    gpiod_direction_input(gpio_descriptors[i]);
		...
		*/
		...
		if (gpiod_set_debounce(gpio_descriptors[i], 200) < 0) {
		...
		gpio_button_irqn[i] = gpiod_to_irq(gpio_descriptors[i]);
		pr_info("IRQ Number = %d\n", gpio_button_irqn[i]);
		if(i == 0){
			if (request_irq(gpio_button_irqn[i],             //IRQ number
				gpio_irq_handler1,   //IRQ handler
					IRQF_TRIGGER_RISING,        //Handler will be called in raising edge
					"button_SW1",               //used to identify the device name using this IRQ
			...
			if (request_irq(gpio_button_irqn[i],             //IRQ number
				gpio_irq_handler2,   //IRQ handler
					IRQF_TRIGGER_RISING,        //Handler will be called in raising edge
					"button_SW2",               //used to identify the device name using this IRQ
					NULL)) {                    //device id for shared IRQ
			    	pr_err("my_device: cannot register IRQ ");
			    	goto err_handle;
			  }
			...
			if (request_irq(gpio_button_irqn[i],             //IRQ number
				gpio_irq_handler3,   //IRQ handler
					IRQF_TRIGGER_RISING,        //Handler will be called in raising edge
					"button_SW3",               //used to identify the device name using this IRQ
					NULL)) {                    //device id for shared IRQ
			    	pr_err("my_device: cannot register IRQ ");
			    	goto err_handle;
			  }
...
void cleanup_module(void)
{
	...
	pwm_disable(pwm_device);
  	for (i = 0; i < NR_GPIO_BUTTONS; i++){
		free_irq(gpio_button_irqn[i], NULL);
