...
int init_module(void)
{
    ...
    ddata[i]->counter = 0;
    ddata[i]->msg_Ptr = NULL;
    ddata[i]->major_minor = MKDEV(major, minor); 
        /* Creating device */
    majMin = MKDEV(major, minor);
    ddata[i]->device = device_create(class, NULL, majMin, ddata[i], "%s%d", DEVICE_NAME, i);
        if (IS_ERR(ddata[i]->device)) {
        pr_err("Device_create failed\n");
        ret = PTR_ERR(ddata[i]->device);
        goto error_device;
    }
    minor++;
}
...
void cleanup_module(void)
{
    ...
    struct device_data* ddata[3];
   for(i = 0; i < 3; i++){
    devices[i] = class_find_device_by_devt(class, MKDEV(major, minor));
    ...
    if (devices[i]) {
        ...
        ddata[i] = dev_get_drvdata(devices[i]);

        if (ddata[i])
            kfree(ddata[i]);

        device_destroy(class, devices[i]->devt);
    }
    minor++;
}
