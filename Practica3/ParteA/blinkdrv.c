			  ...
			  size_t len, loff_t *off)
{
	...
	int retval = 0;
	int i=0;
	unsigned char* messages, *message;
	char *kbuf, *sepMsg;
	unsigned int color, led;

	messages=kmalloc(NR_BYTES_BLINK_MSG*NR_LEDS,GFP_DMA);
	message=kmalloc(NR_BYTES_BLINK_MSG, GFP_DMA);
	kbuf = kmalloc(BUFFER_LENGTH, GFP_KERNEL);
	sepMsg = kmalloc(11, GFP_KERNEL);
	...
	for(i = 0; i < NR_LEDS; i++){
		messages[0+i*NR_BYTES_BLINK_MSG]='\x05';
		messages[1+i*NR_BYTES_BLINK_MSG]=0x00;
		messages[2+i*NR_BYTES_BLINK_MSG]=i;
		messages[3+i*NR_BYTES_BLINK_MSG]=0>>16;
		messages[4+i*NR_BYTES_BLINK_MSG]=0>>8;
		messages[5+i*NR_BYTES_BLINK_MSG]=0;
	}

	if(strcmp(kbuf, "\n\0") != 0){

		while((sepMsg = strsep(&kbuf, ",")) != NULL){
			/* Fill up the message accordingly */
			if(sscanf(sepMsg, "%i:%x", &led, &color) == 2){
				if(led < 0 || led > 7){
					kfree(message);
					kfree(messages);
					kfree(sepMsg);
					kfree(kbuf);
					printk(KERN_INFO "ERROR: No valid led value\n");
					return -EINVAL;
				}
				messages[0+led*NR_BYTES_BLINK_MSG]='\x05';
				messages[1+led*NR_BYTES_BLINK_MSG]=0x00;
				messages[2+led*NR_BYTES_BLINK_MSG]=led; 
				messages[3+led*NR_BYTES_BLINK_MSG]=((color>>16) & 0xff);
	 			messages[4+led*NR_BYTES_BLINK_MSG]=((color>>8) & 0xff);
	 			messages[5+led*NR_BYTES_BLINK_MSG]=(color & 0xff);
			}
			else{
				kfree(message);
				kfree(messages);
	...
	for (i=0;i<NR_LEDS;i++){
		message[0] = messages[0+i*NR_BYTES_BLINK_MSG];
		message[1] = messages[1+i*NR_BYTES_BLINK_MSG];
		message[2] = messages[2+i*NR_BYTES_BLINK_MSG];
		message[3] = messages[3+i*NR_BYTES_BLINK_MSG];
		message[4] = messages[4+i*NR_BYTES_BLINK_MSG];
		message[5] = messages[5+i*NR_BYTES_BLINK_MSG];
		/* 
